...
namespace AssetsTools.NET.Extra
{
    public static class Net35Polyfill
    {
        ...
        public static void CopyToCompat(this Stream input, Stream output, long bytes = -1, ICodeProgress progress = null, int bufferSize = 80 * 1024)
        {
            byte[] buffer = new byte[bufferSize];
            int read;

            //set to largest value so we always go over buffer (hopefully)
            if (bytes == -1) bytes = long.MaxValue;
            ...
            while (bytes > 0 && (read = input.Read(buffer, 0, (int)Math.Min(buffer.Length, bytes))) > 0)
            {
                ...
                if (hasProgress) progress.AddProgress((ulong)read);
            }
        }
    }
}
